...
namespace RCS_Data
{
    ...
    public abstract class DBConnector
    {
        ...
        /// <param name="varParam"></param>
        /// <returns></returns>
        public Object ExecuteScalar(string varSQL, DynamicParameters varParameter = null)
        {
            Object _rc = null;
            DataTable _dataTable = new DataTable();
            try
            {
                if (this.RESPONSE_MSG.status == RESPONSE_STATUS.SUCCESS)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(this.RESPONSE_MSG.message))
                    {
                        this.Open();
                        if (!this.dbTransUsing) this.dbTrans = this.dbConnection.BeginTransaction();
                        _rc = this.dbConnection.ExecuteScalar(varSQL, varParameter, this.dbTrans); 
                    }
                }
            }
            catch (Exception ex)
        ...
        /// 修改
        /// </summary>
        /// <param name="varTableName">資料表</param>
        /// <param name="updateCol">更新欄位</param>
        /// <param name="varWhereSql">where條件</param>
        /// <param name="varParameter">更新參數資料</param>
        /// <returns></returns>
        public int DBExecUpdate(string varTableName, List<string> updateCol, string varWhereSql, DynamicParameters varParameter)
        {
            int eff_row = 0;
            try
            {
                if (this.RESPONSE_MSG.status == RESPONSE_STATUS.SUCCESS)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(this.RESPONSE_MSG.message))
                    {
                        string sql_statment = string.Concat("update ", varTableName,
                       " set ", string.Join(",", updateCol.Select(x => string.Concat(x, "=", this.getNamedArguments(), x)).ToList()),
                       " where ", varWhereSql, ";");
                        this.Open();
                        if (!this.dbTransUsing) this.dbTrans = this.dbConnection.BeginTransaction();
            ...
            try
            {
                if (this.RESPONSE_MSG.status == RESPONSE_STATUS.SUCCESS)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(this.RESPONSE_MSG.message))
                    {
                        this.Open();
                        if (!this.dbTransUsing) this.dbTrans = this.dbConnection.BeginTransaction();
                        eff_row = this.dbConnection.Execute
                            (
                            sql_statment
                            , varWhereParameter
                            , transaction: dbTrans
        ...
        /// <typeparam name="T"></typeparam>
        /// <param name="varTableName">資料表名稱</param>
        /// <param name="varInsertParameter">新增資料參數</param>
        /// <param name="insertCol">新增欄位</param>
        /// <returns></returns>
        public int DBExecInsert<T>(List<T> varInsertParameter, List<string> insertCol)
        {
            int eff_row = 0;
            try
            {
                string varTableName = this.GetClassDisplayName<T>();
                if (this.RESPONSE_MSG.status == RESPONSE_STATUS.SUCCESS)
                {
                    this.RESPONSE_MSG.message = "";
                    if (string.IsNullOrWhiteSpace(this.RESPONSE_MSG.message))
                    {
                        string sql_statment = string.Concat("insert into ", varTableName,
                        " (", string.Join(",", insertCol.Select(x => string.Concat(this.getNamedArguments(), x)).ToList()), ")",
                        " values (", string.Join(",", insertCol), ");");
                        this.Open();
                        if (!this.dbTransUsing) this.dbTrans = this.dbConnection.BeginTransaction();
                        eff_row = this.dbConnection.Execute(sql_statment, varInsertParameter, transaction: dbTrans);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <typeparam name="T"></typeparam>
        /// <param name="varTableName">資料表名稱</param>
        /// <param name="varInsertParameter">傳入物件</param>
        /// <returns></returns>
        public int DBExecInsert<T>(List<T> varInsertParameter)
        {
            int eff_row = 0;
            try
            {
                ...
                string varTableName = this.GetClassDisplayName<T>();
                Dictionary<string, System.Type> valuesList = this.getValues(typeof(T).GetProperties());
                string varInsertSql = string.Format("insert into {0} ({1}) Values ({2});",
                    varTableName,
                    string.Join(",", valuesList.Keys),
                    string.Join(",", valuesList.Keys.Select(x => string.Concat("@", x))));

                // Dictionary<string, System.Type> keysList = this.getKeys(typeof(T).GetProperties());
        ...
        /// <param name="varInsertParameter">傳入物件</param>
        /// <returns></returns>
        public int DBExecUpdate<T>(List<T> varInsertParameter)
        {
            int eff_row = 0;
            try
            {
                ...
                Dictionary<string, System.Type> valuesList = this.getValues(typeof(T).GetProperties());
                Dictionary<string, System.Type> keysList = this.getKeys(typeof(T).GetProperties());
                string varInsertSql = string.Format("UPDATE {0} SET {1} WHERE {2};",
                    varTableName,
                    string.Join(",", valuesList.Keys.Select(x => string.Concat(x, "=@", x))),
                    string.Join(" AND ", keysList.Keys.Select(x => string.Concat(x, "=@", x))));
        ...
        /// <param name="varInsertParameter">傳入物件</param>
        /// <returns></returns>
        public int DBExecDelete<T>(T varInsertParameter)
        {
            int eff_row = 0;
            try
            {
                string varTableName = this.GetClassDisplayName<T>();
                Dictionary<string, System.Type> keysList = this.getKeys(typeof(T).GetProperties());
                string varInsertSql = string.Format("DELETE FROM {0} WHERE {1};",
                    varTableName,
                    string.Join(",", keysList.Keys.Select(x => string.Concat(x, "=@", x))));
        ...
        /// <param name="varInsertParameter">傳入物件</param>
        /// <returns></returns>
        public int DBExecDelete<T>(List<T> varInsertParameter)
        {
            int eff_row = 0;
            try
            {
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        private string GetClassDisplayName<T>()
        {
        ...
        #region 取得物件反射 reflection 
        private Dictionary<string, System.Type> getValues(PropertyInfo[] props)
        {
            Dictionary<string, System.Type> pDic = new Dictionary<string, System.Type>();
            foreach (var prop in props)
            {
                System.Type s = prop.PropertyType;
        ...
        private Dictionary<string, System.Type> getKeys(PropertyInfo[] props)
        {
            Dictionary<string, System.Type> pDic = new Dictionary<string, System.Type>();
            foreach (var prop in props)
            {
